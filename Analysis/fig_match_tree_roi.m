function [ ] = fig_match_tree_roi( site_all, plot_all, match_all )
...
for i = 1:n_site_all;
    ...
    y = [y ;2*match_all{i}.ROI_tree_r];
    if ~isempty(match_all{i}.ROI_roi_r);    
        legend_text{counter} = sprintf('Site %02.0f',site_all(i));
        plot(2*match_all{i}.ROI_roi_r,2*match_all{i}.ROI_tree_r,'x','markersize',10,'linewidth',2,'markeredgecolor',color(i,:));
        counter = counter + 1;
    end
end
