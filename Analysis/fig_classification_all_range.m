function [ ] = fig_classification_all_range( site_all, plot_all, pixel_results_all, xlabel_str )
...
x_arr = pixel_results_all{I}.n_of_r.x;
n_x = numel(x_arr);
...
for i = 1:n_site_all;
    ...
    is_valid(1:numel(pixel_results_all{i}.n_of_r.x)) = true;
    x(i,is_valid) = pixel_results_all{i}.n_of_r.x;
    n_of_FA(i,is_valid) = pixel_results_all{i}.n_of_r.FA;
    n_of_miss(i,is_valid) = pixel_results_all{i}.n_of_r.miss_v;
    n_of_hit(i,is_valid) = pixel_results_all{i}.n_of_r.hit_v;
    n_of_CR(i,is_valid) = pixel_results_all{i}.n_of_r.CR;

    n_v(i,is_valid) = pixel_results_all{i}.n_of_r.v;
    n_o(i,is_valid) = pixel_results_all{i}.n_of_r.o;
    n_vo(i,is_valid) = pixel_results_all{i}.n_of_r.vo;
    n_n(i,is_valid) = pixel_results_all{i}.n_of_r.n;
end
...
legend('False Alarm','Miss','Hit','Correct Rejection');
xlim([xmin xmax ]);
ylim([0 1]);
