function [ match_R, match_t ] = kelbe_analysis_dbh_fun( aux )
...
filepath_anal_dbh_step = sprintf('%sanal_dbh_step.mat', path_mat);
if exist(filepath_anal_dbh_step, 'file');
    return
end
...
for s = 1:n_std;
 %   fprintf('\n Working on sigma %g of %g\n',s,n_step);
    for t = 1:n_trial;
        % Add noise
        P_noise = step_std(s).*randn(1,n_tree(1))';
        P_rad{2} = P_rad{1}.*(1+ P_noise);
        aux.P_rad = P_rad;
...
filepath_anal_dbh_step = sprintf('%sanal_dbh_step.mat', path_mat);
filepath_anal_dbh_RMSE = sprintf('%sanal_dbh_RMSE.mat', path_mat);
save(filepath_anal_dbh_step, 'step_std');
save(filepath_anal_dbh_RMSE, 'match_RMSE');
