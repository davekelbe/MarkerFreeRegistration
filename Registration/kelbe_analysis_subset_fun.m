function [ match_R, match_t ] = kelbe_analysis_subset_fun( aux )
...
filepath_anal_sub_step = sprintf('%sanal_sub_step.mat', path_mat);
if exist(filepath_anal_sub_step, 'file');
  %  return
end
...
for s = 1:n_std;
 %   fprintf('\n Working on sigma %g of %g\n',s,n_step);
    for t = 1:n_trial;
        ...
        P_noise = stdval.*randn(3,n_tree(1));
        P_LCSfull = P_LCS{1} + P_noise;
        P_LCS{2} = P_LCS{1}(:,ix) + P_noise(:,ix);
        P_rad{2} = P_rad{1}(ix);
        aux.P_LCS = P_LCS;
        aux.P_rad = P_rad;
        aux.n_tree = n_tree;
        ...
        match_t = match_t{1,2};
        if ~isempty(match_R);
            P_LCSt =  (match_R*P_LCSfull)+ repmat(match_t,1,n_tree(1));
...
filepath_anal_sub_step = sprintf('%sanal_sub_step.mat', path_mat);
filepath_anal_sub_RMSE = sprintf('%sanal_sub_RMSE.mat', path_mat);
save(filepath_anal_sub_step, 'step_sub');
save(filepath_anal_sub_RMSE, 'match_RMSE');
