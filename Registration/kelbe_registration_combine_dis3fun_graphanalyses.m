function [  ] = kelbe_registration_combine_dis3fun_graphanalyses( aux )
...
for i = 1:n_S;
    for j = 1:n_S;
        if ~isnan(match12_er(i,j));
            ...
            %P_LCSt =  (match12_R{i,j}*P_LCS{j})+ repmat(match12_t{i,j},1,n_tree(j));
            %match12_RMSE(i,j) = sqrt(nanmean(sum((P_LCSt - P_LCS{j}).^2)));
        end
    end
end
