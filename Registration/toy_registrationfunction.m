function [ Rhat, that, m1_best, m2_best ] = toy_registrationfunction(tree1_xyz, tree2_xyz, tree1_r, tree2_r )
...
%tic 
% New method 
error = (eigvala1(row) - eigvala2(col)).^2 +...
    (eigvalb1(row) - eigvalb2(col)).^2;
[~,Isort] = sort(error);
index1s = row(Isort);
index2s = col(Isort);
%fprintf('\n time new = %3.2f\n',toc);
