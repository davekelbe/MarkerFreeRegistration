function [ match_R, match_t ] = kelbe_analysis_pose_fun( aux )
...
info_plot = info_valid_plot{1};
 ...
 filepath_anal_pose_true_rx = sprintf('%sanal_pose_true_rx.mat', path_mat);
if exist(filepath_anal_pose_true_rx, 'file');
    return
end
 ...
 plot = str2double(info_plot);
    ...
    filepath_ply{1} = sprintf('%spoints_full_%03.0f-%02.0f.ply', path_ply{ctr}, info_site, plot);
...
%% Translation, yaw, and roll/pitch 
% Yaw from 0 - 360 
rz_min = 0;
rz_int = 30;
...
rx_min = 0;
rx_int = 30;
t_rxstd = rz_int/4; % 99% within interval 
rx_max = 360;
rx_step = rx_min:rz_int:rx_max;
rx_step_REP = repmat(rx_step',[1,n_trial]);
rx = t_rxstd.*randn(n_step,n_trial) + rx_step_REP;

% Roll from 0 - 360 
ry_min = 0;
ry_int = 30;
t_rystd = rz_int/4; % 99% within interval 
ry_max = 360;
ry_step = ry_min:ry_int:ry_max;
ry_step_REP = repmat(ry_step',[1,n_trial]);
ry = t_rystd.*randn(n_step,n_trial) + ry_step_REP;
...
filepath_anal_pose_true_rx = sprintf('%sanal_pose_true_rx.mat', path_mat);
filepath_anal_pose_true_ry = sprintf('%sanal_pose_true_ry.mat', path_mat);
filepath_anal_pose_true_rz = sprintf('%sanal_pose_true_rz.mat', path_mat);
filepath_anal_pose_true_tx = sprintf('%sanal_pose_true_tx.mat', path_mat);
filepath_anal_pose_true_ty = sprintf('%sanal_pose_true_ty.mat', path_mat);
filepath_anal_pose_true_tz = sprintf('%sanal_pose_true_tz.mat', path_mat);
filepath_anal_pose_match_RMSE = sprintf('%sanal_pose_match_RMSE.mat', path_mat);
save(filepath_anal_pose_true_rx, 'rx');
save(filepath_anal_pose_true_ry, 'ry');
save(filepath_anal_pose_true_rz, 'rz');
save(filepath_anal_pose_true_tx, 'tx');
save(filepath_anal_pose_true_ty, 'ty');
save(filepath_anal_pose_true_tz, 'tz');
save(filepath_anal_pose_match_RMSE, 'match_RMSE');
