...
if ~options_loadvar;
    ...
    %% Parameters
    t_rad = 0.3; %0.06^2; % Trees can be matched if their radius is within threshold
...
n_p = size(perm_of_3a,1);
    ...
    for i = 1:n_S;
        for j = 1:n_S; % Not symmetric
            ...
            for r = 1:n_M1(i,j);
                ...
                figure; hold on; axis equal;
                for p= 1:n_tree1(i);
                    filledCircle([P_LCS1{i}(1,p); P_LCS1{i}(2,p)]',P_rad1{i}(p),1000,P_color(i,:));
                end
                for p= 1:n_tree1(j);
                    filledCircle([P_LCS1{j}(1,p); P_LCS1{j}(2,p)]',P_rad1{j}(p),1000,P_color(j,:));
                end
                plot_triangle(triI);
                plot_triangle(triJ);
                plot_triangle_connection(triI,triJ);
                ...
                [Rhat,that] = rigid_transform_3D(triJ,triI);
                dataJt = ((Rhat*P_LCS1{j})+ repmat(that,1,n_tree1(j)));
                % Add rotation information
                % Use n_match, round-trip or other method to determine fit
                P_LCSIrep = repmat(P_LCS1{i}',[1,1,n_tree1(j)]);
                P_LCSJtrep = zeros(n_tree1(i),3,n_tree1(j));
                for t = 1:n_tree1(i);
                    P_LCSJtrep(t,:,:) = dataJt;
                end
                error_xyz = squeeze(sum((P_LCSIrep-P_LCSJtrep).^2,2));
       ...
       %{
                ...
                for m = 1:numel(m1_best);
                    ...
                    h = filledCircle([(P_LCS1{i}(1,m1) +dataJt(1,m2))/2;...
                         (P_LCS1{i}(2,m1)+ dataJt(2,m2))/2]',3*P_rad1{i}(m1),1000,'y');
                    set(h, 'FaceAlpha', 0.4);
                end
                for p= 1:n_tree1(i);
                    h = filledCircle([P_LCS1{i}(1,p); P_LCS1{i}(2,p)]',P_rad1{i}(p),1000,'r');
                    set(h, 'FaceAlpha', 0.4);
                end
                for p= 1:n_tree1(j);
                    h = filledCircle([dataJt(1,p); dataJt(2,p)]',P_rad1{j}(p),1000,'b');
                    set(h, 'FaceAlpha', 0.4);
                end
                plot_triangle(triI);
                plot_triangle(triJt);
            ...
            if n_match_best > 3;
               % Re-estimate model with inliers 
                [R_reest,t_reest] = rigid_transform_3D(P_LCS1{j}(:,m2_best)',P_LCS1{i}(:,m1_best)');
                match1_R{i,j} = R_reest;
                match1_t{i,j} = t_reest;
    ...
    % match1_rx              (i,j) is pairwise rotation x from j into i
    % match1_ry              (i,j) is pairwise rotation y from j into i
    % match1_rz              (i,j) is pairwise rotation z from j into i
    ...
    for i = 1:n_S; %why -1 before?
        for j = 1:n_S; % Symmetric
            ...
            for r = 1:n_M2(i,j);
                ...
                [Rhat,that] = rigid_transform_3D(triJ,triI);
                dataJt = ((Rhat*P_LCS2{j})+ repmat(that,1,n_tree2(j)));
                % Add rotation information
                % Use n_match, round-trip or other method to determine fit
                P_LCSIrep = repmat(P_LCS2{i}',[1,1,n_tree2(j)]);
                P_LCSJtrep = zeros(n_tree2(i),3,n_tree2(j));
                for t = 1:n_tree2(i);
                    P_LCSJtrep(t,:,:) = dataJt;
                end
                error_xyz = squeeze(sum((P_LCSIrep-P_LCSJtrep).^2,2));
            ...
            if n_match_best > 3;
                % Re-estimate model with inliers       
                [R_reest,t_reest] = rigid_transform_3D(P_LCS2{j}(:,m2_best)',P_LCS2{i}(:,m1_best)');
    ...
    for i = 1:n_S;
        for j = 1:n_S;
            ...
            match12_RMSE(i,j) = nanmean(P_dist);
            end
        end
    end
...
grid on
    ...
    for j = 1:n_S;
        ...
        write2ply(filepath_plyt{j},data2_xyz', 255*P_color(j,:));
    end
    %}
