...
if false;%options_verbose && options_imagepoints;
    ...
    % Run each trial
    for s = 1:n_std;
        fprintf('\n Working on sigma %g of %g',s,n_std);
        for t = 1:n_trial;
            % Add noise
            ...
            [match_R, match_t] = kelbe_registration_function_dis3fun( aux );
    ...
    load('kelbe_registration_wiggle_214.mat');
end
...
figure;
hold on;
scatter(step_std, mean_RMSE,'filled');
xlabel_str = 'Standard deviation of stem locations';
ylabel_str = 'RMSE of registration';
xlabel(xlabel_str);
ylabel(ylabel_str);
axis equal;
plot([0 1.2*s_max],[0 1.2*s_max ])
axis([0 1.2*s_max 0 max(mean_RMSE)]);
