...
all_P_LCSw_pair = cell(n_step, n_trial, n_S);
...
all_P_WCSw = cell(n_step, n_trial, n_S);
    ...
    for trial = 1:n_trial
        for j = 1:n_S;
            ...
            if isvalid_pair(step,trial,j)
                ...
                all_P_LCSw_pair{step,trial,j} = all_R_pair{step, trial}{j}*...
                ...
                all_P_LCSnW_pair{step,trial,j} = all_S_R{step, trial}{w}*all_R_pair{step, trial}{j}*...
                    ...
                    all_S_t{step, trial}{w},...
                    [1 P_ntree(j)]);
                all_P_LCSW_pair{step,trial,j} = all_S_R{step, trial}{w}*all_R_pair{step, trial}{j}*...
                    all_P_LCS{step,trial}{j} + ...
                    repmat(...
                    all_S_R{step, trial}{w}*all_t_pair{step, trial}{j} + ...
                    all_S_t{step, trial}{w},...
            ...
            all_P_WCSw{step,trial,j} = all_S_R{step, trial}{w}'*all_P_WCS{step,trial}{j}-...
                repmat(all_S_R{step, trial}{w}'*all_S_t{step, trial}{w}, [1, P_ntree(j)]);
            % WCS into i
            for i = 1:n_S;
...
true_RMSE_pairnW = nan(n_step, n_trial, n_S);
true_RMSE_pairw = nan(n_step, n_trial, n_S);
true_RMSE_pairnw = nan(n_step, n_trial, n_S);
    ...
    for trial = 1:n_trial
        for j = 1:n_S;
            ...
            if isvalid_pair(step,trial,j);
                ...
                %    sqrt(nanmean(sum((all_P_WCSn{step,trial}{j} -...
                %    all_P_LCSnW_pair{step,trial,j}).^2)));
                true_RMSE_pairw(step,trial,j) = ...
                    sqrt(nanmean(sum((all_P_WCS{step,trial}{j} -...
                    all_P_LCSW_pair{step,trial,j}).^2)));                
            end
            % True pairs
            for i = 1:n_S;
                if isvalid_pairs(step,trial,i,j);
...
%% Pair RMSE out vs RMSE true 
    figure;
    hold on
    isvalid = ~isnan(true_RMSE_pairnw) & ~isnan(all_RMSE_pair);
    scatter(true_RMSE_pairnw(isvalid),all_RMSE_pair(isvalid),5, 'r', 'filled');
    scatter(true_RMSE_pairw(isvalid),all_RMSE_pair(isvalid),5, 'b', 'filled');
...
if options_showfig;
    ...
    t_cut = 1;
    is_valid_MST  = ~isnan(true_RMSE_MST)&~isnan(true_RMSE_pairnw)&...
        true_RMSE_MST<t_cut&true_RMSE_pairnw<t_cut;
    is_valid_WMF  = ~isnan(true_RMSE_WMF)&~isnan(true_RMSE_pairnw)&...
        true_RMSE_WMF<t_cut&true_RMSE_pairnw<t_cut;
    is_valid_SVD  = ~isnan(true_RMSE_SVD)&~isnan(true_RMSE_pairnw)&...
        true_RMSE_SVD<t_cut&true_RMSE_pairnw<t_cut;
    plot(true_RMSE_pairnw(is_valid_MST),true_RMSE_MST(is_valid_MST),'.r')
    plot(true_RMSE_pairnw(is_valid_WMF),true_RMSE_WMF(is_valid_WMF),'.g')
    plot(true_RMSE_pairnw(is_valid_SVD),true_RMSE_SVD(is_valid_SVD),'.b')
    xmin = 0;
    xmax = 0.5;
    ...
    legend(legend_str);
    isnnanpair = ~isnan(true_RMSE_pairnw);
    isvalid = (true_RMSE_pairnw<1);
    %
    % MST
    x = true_RMSE_pairnw(is_valid_MST);
    y = true_RMSE_MST(is_valid_MST);
    %p = polyfit(x,y,1);
    p = polyfitZero(x,y,1);
    yfit = polyval(p,x);
    ...
    plot([xmin xmax],  polyval(p,[xmin xmax]), '-r');
    % WMF
    x = true_RMSE_pairnw(is_valid_WMF);
    y = true_RMSE_WMF(is_valid_WMF);
    % p = polyfit(x,y,1);
    p = polyfitZero(x,y,1);
    yfit = polyval(p,x);
    ...
    plot([xmin xmax],  polyval(p,[xmin xmax]), '-g');
    % SVD
    x = true_RMSE_pairnw(is_valid_SVD);
    y = true_RMSE_SVD(is_valid_SVD);
    %p = polyfit(x,y,1);
    p = polyfitZero(x,y,1);
    yfit = polyval(p,x);
