function [  ] = kelbe_registration_combine_dijkstraposewcs_JVA( aux )
...
fprintf('\nLoad points\n');
% Function DJK 2.14.15 instead of loading, read in unified function JVA 
P_LCS = aux.P_LCSn;
P_rad = aux.P_radn;
P_plot = aux.info_valid_plot;
n_S = numel(P_LCS);
%{
...
t_RMSE = 1;
isoutlier = (match12_RMSE > t_RMSE);
isdiag = match12_RMSE==0;
isnomatch = isnan(match12_RMSE);
isflag = isoutlier | isdiag | isnomatch; 
if sum(isflag(:)) == numel(isflag);
    errordlg('Insufficient matching trees for registration. Revert to manual alignment.');
    return
end
%is_outlier(diag(match12_RMSE)
