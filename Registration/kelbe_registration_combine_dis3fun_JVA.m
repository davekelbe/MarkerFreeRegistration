function [  ] = kelbe_registration_combine_dis3fun_JVA( aux )
    % Only load valid plots
    %info_valid_plot = {'13','15'};
%      info_valid_plot = cell(1,25);
%      for s = 1:5;
%          info_valid_plot{s} = sprintf('%03.0f', s);
%      end
%      info_site = 31;
...
if ~options_loadvar;
    ...
    isvalid = true(size(info_plot));
    
    filepath_P_LCS = sprintf('%s%s', path_mat,'P_LCS.mat');
    filepath_P_rad = sprintf('%s%s', path_mat,'P_rad.mat');
    load(filepath_P_LCS);
    load(filepath_P_rad);

    P_plot = info_plot';
    n_S = sum(isvalid);
    n_tree = cellfun(@numel,P_rad);
    ...
    for d = 1:numel(D);
        ...
        filepath_ply{ctr} = sprintf('%spoints_full_%03.0f-%03.0f.ply', path_ply{ctr}, info_site, plot);
        ctr = ctr + 1;
    end
    isvalid = (P_plot~=0);
    P_LCS = P_LCS(isvalid);
    ...
    if false;%options_verbose && options_imagepoints;
        for s = 1:n_S;
            ...
            % filepath_save = sprintf('%sLCS_%03.0f.eps',path_save, P_plot(s));
...
is_rad = is_ra & is_rb & is_rc;
    ...
    for i = 1:n_S;
       ...
       %         save(sprintf('%sline608.mat', path_matvar);
    %     else
...
for i= 1:n_S;
    ...
    set(gcf, 'color', 'white')
    filepath_fig = sprintf('%spose_c2_%03.0f-%03.0f.png',...
