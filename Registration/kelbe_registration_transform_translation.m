...
tz_int = 5;
tz_max = 20;
tx = tx_min:tx_int:tx_max;
ty = ty_min:ty_int:ty_max;
tz = tz_min:tz_int:tz_max;
n_tx = numel(tx);
n_ty = numel(ty);
n_tz = numel(tz);
...
%tz = tZ(:);
n_s = numel(tz);
t_std = tx_int/4; % 99% within interval 

n_trial = 5;
match_RMSE = zeros(n_s, n_trial);
T = zeros(n_trial,n_s,3);
% Run each trial 
for s = 1:n_s;
    fprintf('\n Working on sigma %g of %g',s,n_s);
    for t = 1:n_trial;
        % Add noise 
        T(t,s,:) = t_std.*randn(3,1) + [tx(s); ty(s); tz(s)] ;
        P_LCS{2} = P_LCS{1} + repmat(squeeze(T(t,s,:)), [1, n_tree(1)]);
        aux.P_LCS = P_LCS;
...
figure; 
hold on;
scatter(tx, mean_RMSE,'filled');
xlabel_str = 'Translation mean, Sigma = interval/4';
...
for s = 1:n_s;
    fprintf('\n Working on sigma %g of %g',s,n_s);
    for t = 1:n_trial;
    R = eye(3);% compose_rotation(rx(r),ry(r),rz(r));
    x_axis_R = (R*x_axis')' + repmat(squeeze(T(t,s,:))', [2,1]);
    y_axis_R = (R*y_axis')' + repmat(squeeze(T(t,s,:))', [2,1]);
    z_axis_R = (R*z_axis')' + repmat(squeeze(T(t,s,:))', [2,1]);
...
%% Only translation by range
tr_min = 0;
tr_int = 5;
tr_max = 20;
tr = tr_min:tr_int:tr_max;
n_tr = numel(tx);
n_s = numel(tr);
t_std = tr_int/4; % 99% within interval 

n_trial = 5;
tR = repmat(tr,[1,n_trial]);
az = 2*pi*rand(1,n_s*n_trial);
el_min = -pi/2;
el_max = pi/2;
el = el_min +(el_max - el_min).*rand(1,n_s*n_trial);
[tx, ty, tz] = sph2cart(az,el,tR);
