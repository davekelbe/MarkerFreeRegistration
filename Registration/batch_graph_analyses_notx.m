...
for step = 1:n_step;
    ...
    for trial = 1:n_trial;
        ...
        graph_analysis_rmsefun_notx( aux )
    end
end
...
all_ij_pairs = nan(n_step,n_trial,n_S,n_S);
...
for step = 1:n_step;
    ...
    for trial = 1:n_trial
        ...
        load(filepath_match_ij);
        all_ij_pairs(step,trial,:,:) = match_ij;
    end
end 
                ...
                if isvalid_pairs(step,trial,i,j);
                 ...
                 %       all_P_LCSni_pairs{step,trial,i,j}).^2)));
                    ...
                    true_RMSE_pairs(step,trial,i,j) = ...
                        ...
                        all_PP_LCSi_pairs{step,trial,i,j}).^2)));
                  %  true_RMSE_pairs(step,trial,i,j) = ...
                  %      sqrt(nanmean(sum((all_P_LCS{step,trial}{i} -...
                  %      all_P_LCSi_pairs{step,trial,i,j}).^2)));
                end
            end
        end
...
%{
    ...
    scatter(true_RMSE_pair(isvalid),all_RMSE_pair(isvalid),5, 'r', 'filled');
    isvalid = ~isnan(true_RMSE_pair) & ~isnan(all_RMSE_pair);
    scatter(true_RMSEn_pairs(isvalid),all_RMSEn_pairs(isvalid),5, 'b', 'filled');
    ...
    hold on
    legend_ctr = 1;
    isvalid = ~isnan(true_RMSEn1_pairs) & ~isnan(all_RMSE_pairs);% & all_RMSE_pairs>0;
    scatter(true_RMSEn1_pairs(isvalid),all_RMSE_pairs(isvalid),5, 'g', 'filled');
    %isvalid = ~isnan(true_RMSEn2_pairs) & ~isnan(all_RMSE_pairs);% & all_RMSE_pairs>0;
    %scatter(true_RMSEn2_pairs(isvalid),all_RMSE_pairs(isvalid),5, 'm');
    isvalid = ~isnan(true_RMSEnn_pairs) & ~isnan(all_RMSE_pairs);% & all_RMSE_pairs>0;
    scatter(true_RMSEnn_pairs(isvalid),all_RMSE_pairs(isvalid),5, 'r', 'filled');
    isvalid = ~isnan(true_RMSE_pairs) & ~isnan(all_RMSE_pairs);% & all_ij_pairs;% & all_RMSE_pairs>0;
...
if options_showfig;
    ...
    isvalid = ~isnan(true_RMSEnn_pairs) & ~isnan(all_RMSE_pairs) & ...
        true_RMSEnn_pairs <1 & all_RMSE_pairs <1 ;
    plot(true_RMSEnn_pairs(isvalid),all_RMSE_pairs(isvalid),'.');
    isvalid = ~isnan(true_RMSEnn_pairs) & ~isnan(all_RMSE_pairs);
    scatter(true_RMSEnn_pairs(isvalid),all_RMSE_pairs(isvalid),5, all_step_pairs(isvalid), 'filled');
