...
[P,~,~] = LM_toy_nest3(P0,data_LCSi(~is_nan,:,:),data_Six(~is_nan), options );
...
is_S1 = (data_Six == 1);
temp_S1 = reshape(P0(ix:end),[(numel(P0)-ix+1)/3,3]);
data_WCSi(is_S1,1,:) = temp_S1(is_S1,:);
for s = 2:n_S;
...
data_WCSf(is_S1,1,:) = temp_S1(is_S1,:);
for s = 2:n_S;
    R = compose_rotation(rf(s,1),rf(s,2),rf(s,3));
    T = tf(s,:)';
    temp_isnotnan = ~isnan(data_LCSi(:,s));
    temp_nisnotnan = sum(temp_isnotnan);
    data_WCSf(temp_isnotnan,s,:) = (R*squeeze(data_LCSi(temp_isnotnan,s,:))'+repmat(T,[1,temp_nisnotnan]))';
end
