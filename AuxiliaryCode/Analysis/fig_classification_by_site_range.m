function [ ] = fig_classification_by_site_range( site_all, plot_all, pixel_results_all, xlabel_str )
...
for i = 1:n_site_all;
    ...
    figure(fh_fa); hold on; plot(pixel_results_all{i}.n_of_r.x,pixel_results_all{i}.p_of_r.FA,'color',color(i,:),'linewidth',2)
    figure(fh_miss); hold on; plot(pixel_results_all{i}.n_of_r.x,pixel_results_all{i}.p_of_r.miss_v,'color',color(i,:),'linewidth',2)
    figure(fh_CR); hold on; plot(pixel_results_all{i}.n_of_r.x,pixel_results_all{i}.p_of_r.CR,'color',color(i,:),'linewidth',2)
    figure(fh_hit); hold on; plot(pixel_results_all{i}.n_of_r.x,pixel_results_all{i}.p_of_r.hit_v,'color',color(i,:),'linewidth',2)
end
