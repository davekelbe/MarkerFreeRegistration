function [  ] = lidar_AS( info_site, info_plot, info_terminate, info_exp )
% Edited for producing visualization for American Scientist
...
% Thresholds
t_max_r = 11.34*sqrt(2); %11.34 is a standard forestry plot size
...
switch info_exp
    case 'harvard'
        path_source = 'D:\Users\djk2312\Documents\Harvard\2012-08-Harvard-Lidar\';
        filename_source = harvard_lookup(info_site, info_plot); %Change path when ssh fixed
    case 'california'
        path_source = 'D:\Users\djk2312\Documents\Harvard\2012-08-Harvard-Lidar\';
        filename_source = california_lookup(info_site, info_plot); %Change path when ssh fixed
end
...
if true;%options_write_ply
    % Full point cloud
    filepath_ply_full  = sprintf('%s%s_%03.0f-%02.0f.ply',path_ply,'points_full',info_site,info_plot);
...
% Define filenames
filepath_I12ieq = sprintf('%s%s_%03.0f-%02.0f%s',path_png,...
...
t_grid_size = .5; % Gridded DEM spacing
...
elseif ~exist(filepath_a_blocked, 'file');
    ...
    save(filepath_xy_blocked, 'xy_blocked');
end
...
if options_write_ply
    filepath_ply_dem = sprintf('ply_dem_%03.0f-%02.0fkelbe3.ply',info_site,info_plot);
    ...
    if ~exist(filepath_ply_dem,'file');
        ...
        write2plyfaces_2( filepath_ply_dem, vertices, plycolor, indices )
...
filepath_I12ieq_outline = sprintf('%sI12ieq_outline_%03.0f-%02.0f.png',path_latex,info_site,info_plot);
...
filepath_I12_farther = sprintf('%sI12_farther_%03.0f-%02.0f.png',path_latex,info_site,info_plot);
...
filepath_registration_R  = sprintf('%s%s',path_mat,'registration_R.mat');
filepath_registration_t  = sprintf('%s%s',path_mat,'registration_t.mat');
filepath_data_x_R  = sprintf('%s%s',path_mat,'data_x_R.mat');
filepath_data_y_R  = sprintf('%s%s',path_mat,'data_y_R.mat');
filepath_data_z_R  = sprintf('%s%s',path_mat,'data_z_R.mat');

if exist(filepath_registration_R,'file');
    load(filepath_registration_t);
    load(filepath_data_x_R);
    load(filepath_data_y_R);
    load(filepath_data_z_R);
elseif ~exist(filepath_registration_R, 'file');
    [registration_sequence] = get_registration_sequence(info_plot);
    registration_sequence = fliplr(registration_sequence);
    [ registration_R, registration_t ] = register_point_cloud2( registration_sequence,...
        path_top);
    data_R = registration_R * ([data_x data_y data_z]') + repmat(registration_t, [1,n_pts]);
    data_x_R = data_R(1,:)';
    data_y_R = data_R(2,:)';
    data_z_R = data_R(3,:)';
    save(filepath_model_z, 'model_z');
    save(filepath_model_y, 'model_y');
    save(filepath_model_r1, 'model_r1');
    save(filepath_model_r2, 'model_r2');
    save(filepath_model_seg, 'model_seg');
    save(filepath_model_color, 'model_color');
end
